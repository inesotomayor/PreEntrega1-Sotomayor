import React, { useState, useEffect } from 'react'
import Item from './Item'

const ItemList = () => {
    const [productos, setProductos] = useState([]) // Variable de estado + función que la actualiza

    useEffect(() => {
        const fetchProductos = () => {
            setTimeout(() => {
                const listadoProductos = [ // Lista de productos:
                    {
                        id: 1,
                        nombre: "To all creatures (2022)",
                        precio: "$85.000,00",
                        foto: 'https://static.wixstatic.com/media/bcfe66_25e5d8b24f6a4412b1add7555acae020~mv2_d_2175_1575_s_2.jpg/v1/fill/w_747,h_536,al_c,q_85,usm_0.66_1.00_0.01,enc_auto/bcfe66_25e5d8b24f6a4412b1add7555acae020~mv2_d_2175_1575_s_2.jpg',
                        descripcion: '30 x 40 cms. - Arches Cold Press 300grs - Schminke Horadam Aquarell, Faber Castell Polychromos y estilógrafo',
                        stock: 1
                    },
                    { id: 2, nombre: "Producto B", foto: '', descripcion: '', precio: "$120.000,00", stock: 2 },
                    { id: 3, nombre: "Producto C", foto: '', descripcion: '', precio: "$90.000,00", stock: 4 },
                    { id: 4, nombre: "Producto D", foto: '', descripcion: '', precio: "$110.000,00", stock: 3 },
                    { id: 5, nombre: "Producto E", foto: '', descripcion: '', precio: "$80.000,00", stock: 5 },
                    { id: 6, nombre: "Producto F", foto: '', descripcion: '', precio: "$100.000,00", stock: 1 },
                    { id: 7, nombre: "Producto G", foto: '', descripcion: '', precio: "$150.000,00", stock: 1 },
                    { id: 8, nombre: "Producto H", foto: '', descripcion: '', precio: "$90.000,00", stock: 4 },
                ]
                setProductos(listadoProductos) // Llamada al listado
            }, 2000) // Delay de 2 seg. para simular carga
        }
        fetchProductos()
    }, []) // Array vacío para que cargue una sola vez

    return (
        <ul className="w-full px-20 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-10">
            {productos.map((producto) =>
                <Item id={producto.id} nombre={producto.nombre} foto={producto.foto} descripcion={producto.descripcion} precio={producto.precio} stock={producto.stock} />
            )}
        </ul>
    )
}

export default ItemList